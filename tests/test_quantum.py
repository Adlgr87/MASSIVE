...
def test_optimizer_returns_required_fields_classical():
    result = qaoa_optimize_interventions(_dummy_objective_function, n_agents=10, n_phases=3, force_classical=True)
    assert {"interventions", "score", "strategy", "qiskit_available"}.issubset(result.keys())


def test_optimizer_matrix_shape_matches_request():
    result = qaoa_optimize_interventions(_dummy_objective_function, n_agents=8, n_phases=4, force_classical=True)
    assert result["interventions"].shape == (4, 8)


def test_optimizer_force_classical_strategy_tag():
    result = qaoa_optimize_interventions(_dummy_objective_function, n_agents=6, n_phases=2, force_classical=True)
    assert result["strategy"] == "classical"


def test_optimizer_deterministic_for_same_seed():
    a = qaoa_optimize_interventions(_dummy_objective_function, n_agents=7, n_phases=3, force_classical=True, seed=123)
    b = qaoa_optimize_interventions(_dummy_objective_function, n_agents=7, n_phases=3, force_classical=True, seed=123)
...
def test_optimizer_rejects_invalid_shape_arguments():
    with pytest.raises(ValueError):
        qaoa_optimize_interventions(_dummy_objective_function, n_agents=0, n_phases=1, force_classical=True)


def test_quantum_wrapper_returns_valid_score():
    result = quantum_optimize_interventions(_dummy_objective_function, n_agents=12, n_phases=2, max_iter=50)
    assert np.isfinite(result["score"])


def test_qiskit_optional_flag_consistent():
    result = qaoa_optimize_interventions(_dummy_objective_function, n_agents=5, n_phases=2)
...
def test_social_architect_drop_in_optimizer_available():
    result = find_optimal_interventions(_dummy_objective_function, n_agents=9, n_phases=3)
