...
def generate_social_network_viz(
    ...
    for i in range(n_nodes):
        if rng.random() < polarizacion_inferida and not amalgama:
            ...
            if is_bipolar:
                extremo = 1.0 if extremo > 0.5 else -1.0
        ...
        op = np.clip(op, -1.0, 1.0) if is_bipolar else np.clip(op, 0.0, 1.0)
    ...
    G.add_edges_from(zip(*np.where(mask), strict=False))
    ...
    edge_trace = go.Scatter(
        x=edge_x, y=edge_y, line=dict(width=0.6, color="#2a364a"), hoverinfo="none", mode="lines"
    )
