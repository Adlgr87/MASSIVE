...
- `Dockerfile`: 3-stage build
  ...
  - Stage `runtime` (python:3.11-slim): installs from wheels (no runtime network), adds nginx + supervisor, copies frontend `dist` to `/usr/share/nginx/html`. Runs **supervisord** as `appuser` managing: `api` (uvicorn, 127.0.0.1:8000), `ui component` (8501), `nginx` (80). Runs as non-root.
- `nginx.conf`: serves `/` from `/usr/share/nginx/html` (SPA `try_files`), proxies `/api/`, `/docs`, `/health`, `/ready`, `/version` → api_backend, `/ui/` → ui component_backend (with Upgrade headers). Static asset caching headers.
- `supervisord.conf`: api+ui component run as `user=appuser`; nginx as root (binds :80).
- `docker-compose.yml`: maps `80:80` (nginx), `8000:8000` (direct API), `8501:8501` (ui component). `.dockerignore` excludes `frontend/node_modules`, `.env`, data, etc.
