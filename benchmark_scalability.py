...
import csv, gc, json, logging, sys, threading, time, tracemalloc
...
log.info("System: %d CPU cores, %.1f GB RAM, %.1f GB available",
         psutil.cpu_count(), RAM_TOTAL / 1e9, RAM_AVAILABLE / 1e9)
...
# ─── Engine runners ──────────────────────────────────────────────────────────
def run_energy_engine(n_agents: int, steps: int, temperature: float,
                      lambda_social: float, seed: int, timeout: float) -> EngineRunResult:
    """EnergyEngine (Langevin 1D) — fastest 1D engine with numba JIT."""
    from energy_engine import SocialEnergyEngine, _step_jit, _SIGMA, NUMBA_AVAILABLE
    rng = np.random.default_rng(seed)
    ...
    try:
        for _ in range(steps):
            ...
            if NUMBA_AVAILABLE:
                opinions = _step_jit(
                    opinions, neighbor_mean, noise, att_pos, att_str,
                    rep_pos, rep_str, lambda_social, eta, sigma2, -1.0, 1.0,
                )
            else:
    ...
    return EngineRunResult(
        "EnergyEngine", n_agents, 0, wall_time=wall, peak_rss_gb=monitor.peak_rss,
...
def run_sparse_multilayer(n_agents: int, steps: int, temperature: float,
                          lambda_social: float, seed: int, timeout: float) -> EngineRunResult:
    ...
    engine = SparseMultilayerEngine(
        layers=layers, interaction_matrix=interaction,
        max_iterations=steps, convergence_threshold=0.0,
    )
    monitor = ResourceMonitor(interval=0.3)
    monitor.start()
    tracemalloc.start()
    ...
    try:
        ...
        tracemalloc.stop()
        monitor.stop()
        return EngineRunResult("SparseMultilayerEngine", n_agents, 0, status="OOM", error_msg="MemoryError")
    except Exception as e:
        tracemalloc.stop()
        monitor.stop()
        return EngineRunResult("SparseMultilayerEngine", n_agents, 0, status="ERROR", error_msg=str(e))
    actual_steps = result.num_steps
    return EngineRunResult(
        "SparseMultilayerEngine", n_agents, 0, wall_time=wall, peak_rss_gb=monitor.peak_rss,
        avg_cpu_pct=monitor.avg_cpu, tracemalloc_peak_mb=peak_py / (1024 ** 2),
        throughput=n_agents * actual_steps / max(wall, 1e-6),
        time_per_step=wall / max(actual_steps, 1) * 1000, status="PASS",
    )


# ─── MassiveSimEngine runner ─────────────────────────────────────────────────
def run_massive_engine(n_agents: int, steps: int, temperature: float,
                       lambda_social: float, seed: int, timeout: float) -> EngineRunResult:
    ...
    try:
        engine = MassiveSimEngine(
            N=n_agents, quantize=True, event_driven=True,
            coupling=lambda_social, dt=0.01, seed=seed,
        )
    ...
    return EngineRunResult(
        "MassiveEngine", n_agents, 0, wall_time=wall, peak_rss_gb=monitor.peak_rss,
        avg_cpu_pct=monitor.avg_cpu, tracemalloc_peak_mb=peak_py / (1024 ** 2),
        throughput=n_agents * steps / max(wall, 1e-6),
        time_per_step=wall / max(steps, 1) * 1000, status="PASS",
    )


# ─── MultilayerEngine (dense baseline) runner ────────────────────────────────
def run_multilayer_dense(n_agents: int, steps: int, temperature: float,
                         lambda_social: float, seed: int, timeout: float) -> EngineRunResult:
    ...
    try:
        engine = MultilayerEngine(
            N=n_agents, layer_weights=(0.4, 0.3, 0.3), coupling=lambda_social,
            dt=0.01, range_type="bipolar", seed=seed,
        )
        ...
        tracemalloc.stop()
        monitor.stop()
        return EngineRunResult("MultilayerEngine", n_agents, 0, status="OOM", error_msg="MemoryError")
    except Exception as e:
        tracemalloc.stop()
        monitor.stop()
        return EngineRunResult("MultilayerEngine", n_agents, 0, status="ERROR", error_msg=str(e))
    return EngineRunResult(
        "MultilayerEngine", n_agents, 0, wall_time=wall, peak_rss_gb=monitor.peak_rss,
        avg_cpu_pct=monitor.avg_cpu, tracemalloc_peak_mb=peak_py / (1024 ** 2),
        throughput=n_agents * steps / max(wall, 1e-6),
        time_per_step=wall / max(steps, 1) * 1000, status="PASS",
    )
...
def run_single(engine_name: str, n_agents: int, iter_idx: int, n_steps: int = STEPS) -> EngineRunResult:
    ...
    if est > SKIP_IF_EXCEEDS_FRAC * avail_gb:
        log.info("  [%s N=%d iter=%d] SKIP_MEM — est %.2f GB > %.2f GB",
                 engine_name, n_agents, iter_idx, est, SKIP_IF_EXCEEDS_FRAC * avail_gb)
        return EngineRunResult(
            engine_name, n_agents, iter_idx, wall_time=0, peak_rss_gb=est,
            avg_cpu_pct=0, status="SKIPPED_MEM",
            error_msg="Est. %.1f GB exceeds memory budget" % est)
    log.info("  [%s N=%d iter=%d] RUNNING (est. %.4f GB)...", engine_name, n_agents, iter_idx, est)
    result_store: dict = {}
    _run_with_timeout(
        runner, (n_agents, n_steps, TEMPERATURE, LAMBDA_SOCIAL, SEED, TIMEOUT_PER_RUN),
        TIMEOUT_PER_RUN + 20, result_store)
    outcome = result_store.get("result", ("error", "no result"))
    status_code, payload = outcome
    if status_code == "timeout":
        log.warning("  [%s N=%d iter=%d] TIMEOUT (>%ds)", engine_name, n_agents, iter_idx, TIMEOUT_PER_RUN)
        return EngineRunResult(engine_name, n_agents, iter_idx, wall_time=TIMEOUT_PER_RUN,
                               status="TIMEOUT", error_msg="Exceeded timeout")
    elif status_code == "oom":
        log.warning("  [%s N=%d iter=%d] OOM (MemoryError in subprocess)", engine_name, n_agents, iter_idx)
        return EngineRunResult(engine_name, n_agents, iter_idx, status="OOM", error_msg="MemoryError")
    elif status_code == "error":
        log.error("  [%s N=%d iter=%d] ERROR: %s", engine_name, n_agents, iter_idx, payload)
        return EngineRunResult(engine_name, n_agents, iter_idx, status="ERROR", error_msg=str(payload))
    else:
        r: dict = payload
        return EngineRunResult(
            engine=engine_name, n_agents=n_agents, iteration=iter_idx,
            wall_time=r.get("wall_time", 0), peak_rss_gb=r.get("peak_rss_gb", 0),
            avg_cpu_pct=r.get("avg_cpu_pct", 0), tracemalloc_peak_mb=r.get("tracemalloc_peak_mb", 0),
            throughput=r.get("throughput", 0), time_per_step=r.get("time_per_step", 0),
...
def main():
    ...
    log.info("Steps: %d, Temperature: %.1f, Lambda: %.1f, Seed: %d", STEPS, TEMPERATURE, LAMBDA_SOCIAL, SEED)
    log.info("Iterations per config: %d, Timeout: %ds", NITERS, TIMEOUT_PER_RUN)
    log.info("Populations: %s",
    ...
    for engine_name in engines:
        ...
        for n_agents in POPULATIONS:
            ...
            for iter_idx in range(n_iters):
                ...
                log.info("    iter=%d: %s, time=%.4fs, ram=%.4fGB, cpu=%.1f%%, thr=%.0f ag/s",
                         iter_idx, r.status, r.wall_time, r.peak_rss_gb, r.avg_cpu_pct, r.throughput)
                csv_rows.append({
                    "engine": r.engine, "n_agents": r.n_agents, "iteration": r.iteration,
                    "steps": n_steps,
                    "wall_time_s": round(r.wall_time, 4), "peak_rss_gb": round(r.peak_rss_gb, 4),
            ...
            fail_count = sum(1 for r in iter_results if r.status in ("OOM", "TIMEOUT", "SKIPPED_MEM"))
            if fail_count >= max(1, n_iters // 2):
                log.warning("  [%s] %d/%d failures at %s — stopping progression",
                            engine_name, fail_count, NITERS, label)
        ...
        for s in summary_data:
            if s["engine"] == eng_name and s["status"] == "PASS" and s["time_median_s"] > 0:
                if best is None or s["n_agents"] > best["n_agents"]:
                    best = s
        if best:
            projections[eng_name] = extrapolate(best["n_agents"], best["time_median_s"],
                                               best["peak_ram_median_gb"])

    # ─── Save JSON ────────────────────────────────────────────────────────
    json_data = {
        "metadata": {
            "timestamp": time.strftime("%Y-%m-%dT%H:%M:%S"),
            "steps": STEPS, "temperature": TEMPERATURE, "lambda_social": LAMBDA_SOCIAL,
            "seed": SEED, "iterations_default": NITERS, "iterations_adaptive": True, "timeout_seconds": TIMEOUT_PER_RUN, "base_steps": STEPS, "adaptive_steps": True,
            "gpu_available": GPU_AVAILABLE, "gpu_backend": GPU_BACKEND,
        ...
        "raw_results": [
            {"engine": r.engine, "n_agents": r.n_agents, "iteration": r.iteration,
             "wall_time_s": r.wall_time, "peak_rss_gb": r.peak_rss_gb,
             "avg_cpu_pct": r.avg_cpu_pct, "tracemalloc_peak_mb": r.tracemalloc_peak_mb,
             "throughput_agents_s": r.throughput, "time_per_step_ms": r.time_per_step,
             "status": r.status, "error_msg": r.error_msg}
    ...
    # ─── Save CSV ─────────────────────────────────────────────────────────
    csv_fields = ["engine", "n_agents", "iteration", "steps", "wall_time_s", "peak_rss_gb", "avg_cpu_pct",
                  "tracemalloc_peak_mb", "throughput_agents_s", "time_per_step_ms", "status", "error_msg"]
    ...
    log.info("=" * 70)
    log.info(f"{'Engine':<28} {'Pop':<8} {'Time(s)':<12} {'PeakRAM(GB)':<14} {'Thr(ag/s)':<14} {'t/step(ms)':<14} {'Status':<12}")
    log.info("-" * 100)
    for s in summary_data:
        log.info(f"{s['engine']:<28} {s['label']:<8} {s['time_median_s']:<12.4f} "
    ...
    for eng, proj in projections.items():
        log.info("  %s: %.2f hours (%.1f days), %.1f TB RAM, %.0f kWh",
                 eng, proj["estimated_time_hours"], proj["estimated_time_days"],
                 proj["estimated_ram_tb"], proj["energy_estimate_kwh"])
