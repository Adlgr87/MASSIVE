...
class SimulationState:
    ...
    opinion: np.ndarray | None = None
    cooperation: np.ndarray | None = None
    hierarchy: np.ndarray | None = None
    income: np.ndarray | None = None
    info_access: np.ndarray | None = None
    ...
    def __post_init__(self):
        ...
        if all(
            v is None
            for v in [self.opinion, self.cooperation, self.hierarchy, self.income, self.info_access]
        ):
    ...
    def copy(self) -> "SimulationState":
        ...
        return SimulationState(
            ...
            attributes=(
                {k: v.copy() for k, v in self.attributes.items()}
                if self.attributes is not None
                else None
            ),
...
class SimulationConfig:
    ...
    M: int | None = None
    layer_weights: tuple[float, ...] | list[float] | None = None
    ...
    def __post_init__(self):
        ...
        if self.layer_weights is not None:
            ...
            if len(weights) != self.K:
                raise ValueError(f"layer_weights must have length K={self.K}, got {len(weights)}")
            if weights.sum() <= 0:
...
def legacy_dict_to_simulation_state(legacy_dict: dict[str, Any]) -> SimulationState:
    ...
    for key, value in legacy_dict.items():
        if (
            key not in legacy_to_canonical
