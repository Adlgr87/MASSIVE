...
class LayerState:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "graph_adjacency": (
                self.graph_adjacency.toarray().tolist()
...
class MultilayerState:
    ...
    def from_layers(
        cls,
        layers: list[LayerState],
        inter_layer_edges: np.ndarray | None = None,
    ) -> MultilayerState:
...
class SparseMultilayerEngine:
    ...
    def remove_layer(self, layer_idx: int) -> None:
        ...
        if self.inter_layer_edges.size > 0:
            mask = (self.inter_layer_edges[:, 0] != layer_idx) & (
    ...
    def add_inter_layer_edge(
        self, src_layer: int, src_node: int, dst_layer: int, dst_node: int
    ) -> None:
        """
        ...
        self.inter_layer_edges = (
            np.vstack([self.inter_layer_edges, edge]) if self.inter_layer_edges.size > 0 else edge
        )
    ...
    def _build_full_adjacency(self) -> csr_matrix:
        ...
        offset = 0
        for i, (layer, _state) in enumerate(zip(self.layers, self._current_states, strict=False)):
...
class StabilityAnalyzer:
    ...
    def __init__(self, engine: SparseMultilayerEngine | None = None):
        self.engine = engine

    def analyze(self, engine: SparseMultilayerEngine | None = None) -> dict[str, Any]:
        """
