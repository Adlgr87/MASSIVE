...
def _profile_value(category: str, param_id: str, cultural_profile: str) -> float:
    entry = MASSIVE_EMPIRICAL_MASTER[category][param_id]
...
def build_empirical_engine_config(cultural_profile: str = "mixed") -> dict:
    ...
    homophily = _clamp(_profile_value("network_dynamics", "HOMOFILIA_RED", cultural_profile), 0.0, 1.0)
    confirmation_bias = _clamp(_profile_value("individual_psychology", "SESGO_CONFIRMACION", cultural_profile), 0.0, 1.0)
    viral_amplification = _clamp(_profile_value("network_dynamics", "AMPLIFICACION_VIRAL", cultural_profile), 0.0, 1.0)

    return {
        "ruido_base": round(_scale_unit_to_range(runtime["temperature"], RUIDO_BASE_MIN, RUIDO_BASE_MAX), 4),
        "ruido_desconfianza": round(_scale_unit_to_range(runtime["narrative_decay_rate"], RUIDO_DESCONFIANZA_MIN, RUIDO_DESCONFIANZA_MAX), 4),
        "efecto_vecinos_peso": round(_scale_unit_to_range(runtime["social_influence_lambda"], EFECTO_VECINOS_MIN, EFECTO_VECINOS_MAX), 4),
        "alpha_blend": round(_scale_unit_to_range(runtime["attractor_depth"], ALPHA_BLEND_MIN, ALPHA_BLEND_MAX), 4),
        "sesgo_confirmacion": round(confirmation_bias, 4),
        "hk_epsilon": round(HK_EPSILON_MIN + (HK_EPSILON_MAX - HK_EPSILON_MIN) * (1.0 - homophily), 4),
        "competencia_peso": round(_scale_unit_to_range(viral_amplification, COMPETENCIA_PESO_MIN, COMPETENCIA_PESO_MAX), 4),
        "umbral_media": TIPPING_POINT_MEAN,
        "umbral_std": TIPPING_POINT_STD,
        "homofilia_tasa": round(_scale_unit_to_range(homophily, HOMOFILIA_TASA_MIN, HOMOFILIA_TASA_MAX), 4),
        ...
        "strategic": {
            ...
            "strategic_weight": round(_scale_unit_to_range(abs(runtime["repeller_strength"]), STRATEGIC_WEIGHT_MIN, STRATEGIC_WEIGHT_MAX), 4),
        },
    }
...
def apply_empirical_profile(cfg: dict) -> dict:
    ...
    merged["_empirical_profile"] = MASSIVE_EMPIRICAL_MASTER["meta"]["version"]
    return merged
...
def export_to_json(path: str | None = None) -> str:
    ...
    payload = {
        "master": MASSIVE_EMPIRICAL_MASTER,
        "runtime_params": MASSIVE_RUNTIME_PARAMS,
