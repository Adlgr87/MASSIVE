...
class ValidationReport:
    ...
    def _calculate_overall_score(self):
        ...
        for result in self.results:
            ...
            if any(
                ...
                for word in ["population", "age", "literacy", "fertility", "life"]
            ):
                category_scores["demographic"].append(result.score)
            elif any(
                word in metric_lower
                for word in ["gdp", "gini", "income", "unemployment", "budget", "sector"]
            ):
                category_scores["economic"].append(result.score)
            elif any(
                word in metric_lower for word in ["ethnic", "religion", "language", "urbanization"]
            ):
    ...
    def save(self, path: str | Path | None = None):
        ...
        if path is None:
            path = Path(
                f"reports/factbook_validation_{self.country_code}_{self.timestamp[:10]}.json"
            )
...
class FactbookValidator:
    ...
    def validate_simulation(
        ...
        try:
            ...
            if real_data:
                ...
                log.warning(
                    f"[FactbookValidator] No se encontraron datos reales para {country_identifier}"
                )
    ...
    def _validate_demographics(
        ...
        if sim_pop is not None and real_pop is not None and sim_pop > 0 and real_pop > 0:
        ...
        sim_age = self._extract_metric(
            sim_results, ["age_structure", "age_distribution", "demographics"]
        )
    ...
    def _validate_economics(
        ...
        sim_gdp_capita = self._extract_metric(
            sim_results, ["gdp_per_capita", "avg_income", "mean_income"]
        )
        ...
        if isinstance(sim_sectors, dict):
            ...
            sim_agriculture = sim_sectors.get(
                "agriculture", sim_sectors.get("agriculture_share", 0)
            )
    ...
    def _validate_social(
        ...
        sim_ethnic_diversity = self._extract_metric(
            sim_results, ["ethnic_diversity", "ethnic_diversity_index"]
        )
        ...
        sim_religious_diversity = self._extract_metric(
            sim_results, ["religious_diversity", "religion_diversity_index"]
        )
        ...
        sim_language_diversity = self._extract_metric(
            sim_results, ["language_diversity", "language_diversity_index"]
        )
    ...
    def _extract_metric(
        ...
        for key in possible_keys:
            if key in data:
                return data[key]
            # Try nested access
            for _top_key, top_value in data.items():
    ...
    def compare_opinion_distributions(
        ...
        real_distribution: np.ndarray | None = None,
        country_identifier: str | None = None,
        bins: int = 10,
    ) -> dict[str, Any]:
        """
        ...
        if real_distribution is None and country_identifier:
            self.context.get_country(country_identifier)
    ...
    def set_tolerance_level(self, level: str):
        ...
        if level in self._tolerance_levels:
            ...
            warnings.warn(f"Unknown tolerance level: {level}. Using 'moderate'.", stacklevel=2)
