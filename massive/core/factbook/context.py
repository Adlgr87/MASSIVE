...
class FactbookContext:
    ...
    def __init__(self, data_path: str | None = None):
        ...
        self.current_country: CountryData | None = None
    ...
    def _try_load_data(self):
        ...
        try:
            ...
            log.warning(
                "[FactbookContext] FactbookDataLoader no disponible. Usando datos por defecto."
            )
            self._initialized = True
    ...
    def _extract_data(self, country: CountryData):
        ...
        if isinstance(ethnic_data, dict):
            country.ethnic_groups = {
                k: float(v.replace("%", "").strip() if isinstance(v, str) else v)
        ...
        country.life_expectancy = float(
            get_nested(raw, ["people", "life_expectancy_at_birth"], 75.0)
        )
        ...
        # Sector shares
        country.agriculture_share = float(
            get_nested(raw, ["economy", "gdp_composition_by_sector", "agriculture"], 5.0)
        )
        country.industry_share = float(
            get_nested(raw, ["economy", "gdp_composition_by_sector", "industry"], 25.0)
        )
        country.services_share = float(
            get_nested(raw, ["economy", "gdp_composition_by_sector", "services"], 70.0)
        )
        ...
        country.budget_expenditures = float(
            get_nested(raw, ["economy", "budget", "expenditures"], 1.1e12)
        )
        country.budget_surplus_deficit = float(
            get_nested(raw, ["economy", "budget", "surplus_or_deficit"], -0.1e12)
        )

        # Political data
        country.government_type = str(
            get_nested(raw, ["government", "country_name", "conventional_long_form"], "democratic")
        )
    ...
    def get_country(self, country_identifier: str) -> CountryData | None:
        """
    ...
    def get_social_pressure_weights(self, country_identifier: str) -> dict[str, float]:
        ...
        return params.get(
            "social_pressure_weights", {"ethnic": 0.5, "religious": 0.5, "language": 0.5}
        )
...
_factbook_context: FactbookContext | None = None


def get_factbook_context(data_path: str | None = None) -> FactbookContext:
    """
