...
_KEYWORDS = {
    ...
    "campana": [
        ...
        r"campaign",
        r"advertis",
        r"bombardeo medi[aá]tico",
    ],
    "desconfianza": [
        r"desconf[ií]an",
        r"desconf[ií]a",
        r"descr[eé]dit",
        r"instituciones (desacreditadas|d[eé]biles|en crisis)",
        r"corrupci",
        r"crisis de confianza",
        r"distrust",
        r"mistrust",
        r"trust in",
...
_RULE_HINTS = {
    "hk": [
        r"\bhk\b",
        r"confianza acotada",
        r"bounded confidence",
        r"solo escuchan a",
        r"mismos c[ií]rculos",
    ],
    "backlash": [
        r"backlash",
        r"efecto rebote",
        r"reactancia",
        r"reacci[oó]n adversa",
        r"boomerang",
        r"contraproducente",
        r"rechazan la propaganda",
    ],
    "replicador": [
        r"teor[ií]a de juegos",
        r"replicador",
        r"evolutionary game",
        r"estrategias",
        r"juego de la",
        r"dilema del prisionero",
        r"payoff",
        r"game theory",
    ],
...
def interpret(
    ...
    *,
    ...
    pct = _parse_number(
        description, [r"(\d{1,3})\s*%", r"(\d{1,2}(?:\.\d+)?)\s*por ciento", r"(\d{1,3})\s*percent"]
    )
    if pct is not None:
    ...
    if groups["campana"]:
        fuerte = bool(
            re.search(r"agresiv|intens|masiv|fuerte|fuerte|heavy|strong|massive", text_low)
        )
        ...
        assumptions.append(
            AssumptionItem(
                ...
                reason="hay una campaña activa"
                + (" descrita como agresiva/intensa" if fuerte else ""),
                confidence=0.65,
            )
        )
    else:
    ...
    if groups["grupos"]:
        ...
        questions.append(
            (
                ...
                if lang == "es"
                else "Do the groups you mention hold opposing positions, or just different levels of support?"
            ),
        )
    ...
    if lang == "es":
        if not groups["rechazo"] and not groups["campana"]:
            questions.append(
                "¿Quién impulsa el cambio de opinión: una campaña, un evento, o la conversación entre personas?"
            )
        if pct is None:
            questions.append(
                "Si conoces el apoyo actual aproximado (%), dímelo y lo usaré como punto de partida."
            )
        questions.append(
            "¿En qué plazo te interesa el resultado: semanas, meses o un ciclo electoral completo?"
        )
    else:
        if not groups["rechazo"] and not groups["campana"]:
            questions.append(
                "What is driving opinion change: a campaign, an event, or people talking to each other?"
            )
        if pct is None:
            questions.append(
                "If you know the current support level (%), tell me and I will use it as the starting point."
            )
        questions.append(
            "What time horizon matters to you: weeks, months, or a full election cycle?"
        )
    ...
    if lang == "es":
        ...
        if groups["polarizacion"]:
            reply_parts.append(
                "Because there are polarization signals, I raised the confirmation bias."
            )
        if mechanism:
