...
Dos casos se consideran **independientes** si **ambas** condiciones se cumplen:
...
2. **No solapamiento temporal:** sus ventanas de tiempo no coinciden con el mismo shock global no modelado (p.ej., una pandemia mundial).  
   Si comparten un shock, deben compartir un `cluster_id` en `meta.json` y la evaluación debe hacerse a **nivel de cluster** (un test DM por cluster, no por caso), corrigiendo por el tamaño muestral efectivo reducido.

### 2.2 Uso de `cluster_id`

`cluster_id` agrupa casos afectados por el mismo confusor estructural o temporal.  
Reglas:

- Establecer `cluster_id` en `meta.json` cuando los casos comparten plataforma, evento regional o ventana temporal solapada.
- Los tests estadísticos y tamaños de efecto se computan **por cluster** cuando N_cluster > 1; los resultados individuales se reportan como suplementarios.
