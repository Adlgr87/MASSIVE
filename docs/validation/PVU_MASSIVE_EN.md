# MASSIVE Protocol of Validated Use (PVU-BS) — English

> **Version:** 1.0 · **Status:** Active  
> **Spanish version:** [PVU_MASSIVE_ES.md](PVU_MASSIVE_ES.md)

---

## 1. Purpose

The **Protocol of Validated Use (PVU-BS)** establishes the minimum evidence standard required to claim that MASSIVE provides *validated* predictive performance on real-world opinion dynamics data.
...
MASSIVE is validated on a **compound target** that captures both level and dynamics of opinion polarization:
...
1. Looking at test metrics (even aggregated) before freezing model configuration.
2. Adjusting prompts, temperature, provider, regime rules, or any parameter **after** seeing test results, even informally.
3. Selecting or discarding cases *post-hoc* to improve reported results.
4. Running the model multiple times on the same test set and reporting the best run (unless explicitly doing an LLM consistency check under §6).
5. Using test observations as training context in the LLM prompt.

**Registry:** A pre-registration document (see `preregistration_template_EN.md`) must be committed to the repository **before** breaking the test seal.
...
> MASSIVE produces significantly lower MAE on the held-out test set compared to the naive baseline (last-value persistence), after Holm–Bonferroni correction.

### 4.2 Test Procedure

1. Compute forecasts for **all** baselines and MASSIVE on the test split of **each** case.
2. For each case, run a two-sided **Diebold–Mariano (DM) test** of MASSIVE vs each baseline using squared-error loss.
...
| B2 | Moving Average | Mean of the last 4 observations |
| B3 | AR(1) | First-order autoregression fitted by OLS |
| B4 | Random Regime | Random walk with noise calibrated from training |
...
When MASSIVE is run in LLM mode:
...
```bash
# Offline mode (no API key — default for CI):
...
| Term | Definition |
|------|-----------|
| PVU-BS | Protocol of Validated Use — MASSIVE |
