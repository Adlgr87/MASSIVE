...
class EnsembleConfig(BaseModel):
    ...
    variations: list[SimVariation] = Field(
...
class BifurcationMap(BaseModel):
    ...
    param_importances: dict[str, float] = Field(description="Importancia de cada parámetro (0-1)")
    transition_costs: list[dict] = Field(
        default_factory=list, description="Costo de mover el grupo entre familias"
    )
...
class EnsembleResult(BaseModel):
    ...
    stability: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Qué tan estable es la dinámica"
    )
    summary: str = ""
